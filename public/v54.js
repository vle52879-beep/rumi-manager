...
(() => {
  ...
  employeeForm = function employeeFormV54(x = null) {
    ...
    return `<form class="form-grid" data-form="${x?'employee-edit':'employee-create'}" data-id="${x?.id||''}">
      ...
      <div class="field"><label>Giờ tối đa/tuần</label><input type="number" step="0.5" name="max_weekly_hours" min="1" max="56" value="${esc(x?.max_weekly_hours||56)}"></div><div class="field"><label>Giờ tối đa/ngày</label><input type="number" step="0.5" name="max_daily_hours" min="1" value="${esc(x?.max_daily_hours||8)}"></div>
